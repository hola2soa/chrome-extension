...
SearchPage.onEnter = (store) => (nextState, replaceState, callback) => {
  const auth = store.getState().member;
  const tasks = auth.user.stores.map(shop => store.dispatch(fetchPart(shop.name, 'tops')));
  tasks.push(store.dispatch(fetchPinned({email_address: auth.user.email_address})));
  Promise.all(tasks).then();
  callback();
};

SearchPage.onLeave = (store) => () => {
